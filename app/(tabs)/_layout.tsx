...
const TabIcon = ({ icon, color, name, focused, circular }: TabIconTypes) => {
  return (
    <View className={`items-center justify-center gap-2 ${circular ? "rounded-full bg-[#32D74B] p-3" : ""}`}>
      <Image
        source={icon}
        resizeMode="contain"
        className="w-6 h-6"
      ...
      {!circular && (
        <Text
          className={`${focused ? "font-psemibold text-[#32D74B]" : "font-pregular text-white"} text-xs`}
        >
          {name}
        </Text>
      )}
